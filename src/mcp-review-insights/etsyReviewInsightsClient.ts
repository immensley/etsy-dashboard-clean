...
export interface FetchReviewInsightsParams {
  shopId: string;
  limit?: number;
  offset?: number;
}
...
function buildUrl({ shopId, limit, offset }: FetchReviewInsightsParams): string {
  ...
  baseUrl.searchParams.set("include_feedback", "true");

  return baseUrl.toString();
}

export async function fetchReviewInsights(params: FetchReviewInsightsParams): Promise<unknown> {
  if (!params.shopId) {
  ...
  console.log("Review Insights ZenRows URL:", zenrowsUrl); // DEBUG
  ...
  console.log("Review Insights RAW RESPONSE (first 500 chars):", text.slice(0, 500)); // DEBUG

  try {
    return JSON.parse(text);
  } catch (err) {
    const message = err instanceof Error ? err.message : String(err);
    throw new Error("Failed to parse review insights JSON from ZenRows: " + message);
  }
}
