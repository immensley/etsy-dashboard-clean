...
type TimeseriesValue = RawRecord | [unknown, unknown] | undefined;
...
export interface ShopMetricSummary {
  key: string;
  label?: string;
  unit?: string;
  current?: number;
...
function pickString(...values: unknown[]): string | undefined {
  for (const value of values) {
    const result = toString(value as MaybeString);
    if (result) {
      return result;
    }
  }
...
function pickNumber(...values: unknown[]): number | undefined {
  for (const value of values) {
    const result = toNumber(value as MaybeNumber);
    if (result !== undefined) {
      return result;
    }
  }
...
function parseTimeseriesEntry(entry: TimeseriesValue): MetricTimeseriesPoint | undefined {
  if (!entry) {
    return undefined;
  }
  ...
  if (Array.isArray(entry) && entry.length >= 2) {
    ...
    const date = pickString(dateValue) ?? `${dateValue}`;
  ...
  if (typeof entry === "object") {
    ...
    if (date && value !== undefined) {
      ...
      const point: MetricTimeseriesPoint = { date, value };
...
function extractTimeseries(raw: RawRecord): MetricTimeseriesPoint[] | undefined {
  const candidates = [raw.trend, raw.timeseries, raw.series, raw.history, raw.points, raw.data];
  for (const candidate of candidates) {
    if (Array.isArray(candidate)) {
      const parsed = candidate
        .map((entry) => parseTimeseriesEntry(entry as TimeseriesValue))
        .filter((entry): entry is MetricTimeseriesPoint => Boolean(entry));
      if (parsed.length > 0) {
        return parsed;
      }
    }
...
function extractCurrent(raw: RawRecord): number | undefined {
  return pickNumber(raw.current, raw.value, raw.total, raw.count, raw.latest);
}

function extractPrevious(raw: RawRecord): number | undefined {
  return pickNumber(raw.previous, raw.prior, raw.comparison, raw.last_period, raw.last);
}

function extractUnit(raw: RawRecord): string | undefined {
...
function computeChange(current: number | undefined, previous: number | undefined): { change?: number; rate?: number } {
...
function parseMetricEntry(key: string, value: unknown): ShopMetricSummary | undefined {
  ...
  const label = pickString(record.label, record.display_name, record.name) ?? humanizeKey(key);
  ...
  const summary: ShopMetricSummary = {
    key,
    label,
    raw: record,
  };
...
function collectMetricSources(data: unknown): RawRecord[] {
  const sources: RawRecord[] = [];
  if (!data || typeof data !== "object") {
    return sources;
  }

  const root = data as RawRecord;
    ...
    if (Array.isArray(candidate)) {
      for (const entry of candidate) {
        if (entry && typeof entry === "object") {
          sources.push(entry as RawRecord);
        }
      }
      continue;
    }

    sources.push(candidate as RawRecord);
  }

  return sources;
}

export function parseShopMetrics(data: unknown): ShopMetricSummary[] {
  const sources = collectMetricSources(data);
  const results: ShopMetricSummary[] = [];
