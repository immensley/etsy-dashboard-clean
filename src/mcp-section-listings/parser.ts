...
type RawResponseObject = { listings?: RawListing[]; results?: RawListing[] };
...
export interface PricingSummary {
  ...
  amount_unformatted?: string | number;
  amount_int?: number;
...
export interface DiscountSummary {
  price?: string | number;
  description?: string;
  description_unescaped?: string;
  price_int_usd?: number;
  price_unformatted?: string | number;
}

export interface DeliverySummary {
  free_shipping_countries?: string[];
  free_shipping_data?: Record<string, unknown>;
  transit_details_manual?: Record<string, unknown>[];
  transit_details_calculated?: Record<string, unknown> | null;
  ships_to_regions?: string[];
  origin_country_id?: number;
  origin_postal_code?: string;
  min_processing_days?: number;
  max_processing_days?: number;
  is_buyer_promise_eligible?: boolean;
}

export interface FlagsSummary {
  is_machine_translated?: boolean;
  is_bestseller?: boolean;
  is_bestseller_leaf?: boolean;
  is_bestseller_l3?: boolean;
  is_scarce?: boolean;
  is_handmade?: boolean;
  is_customizable?: boolean;
  is_personalizable?: boolean;
  has_variations_with_pricing?: boolean;
  is_vintage?: boolean;
  is_unique?: boolean;
  is_in_merch_library?: boolean;
  is_surfaceable?: boolean;
  for_pattern_consumption?: boolean;
  is_vacation?: boolean;
  is_pattern?: boolean;
  is_retail?: boolean;
  has_manually_adjusted_thumbnail?: boolean;
  is_listing_image_landscape?: boolean;
  is_top_rated?: boolean;
  has_star_seller_signal?: boolean;
  can_be_waitlisted?: boolean;
  is_private?: boolean;
  should_show_buy_it_now_button?: boolean;
  is_made_to_order?: boolean;
  are_returns_accepted?: boolean;
  are_exchanges_accepted?: boolean;
}
...
export interface VariationSummary {
  num_color_variations?: number;
  num_size_variations?: number;
  variations_data?: Record<string, unknown>;
  promotions?: Record<string, unknown> | Record<string, unknown>[];
  promotion_data?: Record<string, unknown>;
  signal_pecking_order?: Record<string, unknown>[] | string[];
  style_attributes?: Record<string, unknown>[];
  suppression_restrictions?: string[];
  country_specific_pricing?: Record<string, unknown>;
  lowest_purchasable_price?: Record<string, unknown>;
}

export interface ParsedSectionListing {
...
function collectImages(listing: RawListing): ListingMediaSummary | undefined {
  const media: ListingMediaSummary = {};
  const primaryImage = toString(listing.image) ?? toString(listing.image170);
  if (primaryImage) {
    media.primary_image = primaryImage;
  }

  const fromImg = listing.img as Record<string, unknown> | undefined;
  ...
  const listingImages = listing.listing_images as unknown;
  ...
  const imageKeys = listing.images as unknown;
...
function collectShop(listing: RawListing): ShopSummary | undefined {
  ...
  const inlineShop = listing.shop as Record<string, unknown> | undefined;
  if (inlineShop) {
    const inlineName = toString(inlineShop.store_name ?? inlineShop.shop_name);
...
function collectLogging(listing: RawListing): Record<string, unknown> | undefined {
  const logging = listing.logging;
  if (logging && typeof logging === "object") {
    return logging as Record<string, unknown>;
  }
  return undefined;
}

function collectPricing(listing: RawListing): PricingSummary | undefined {
  ...
  const unformatted = listing.price_unformatted as string | number | undefined;
  ...
  const logging = listing.logging as Record<string, unknown> | undefined;
  if (logging) {
    const sellerCurrency = toString(logging.seller_currency);
    if (sellerCurrency) {
      summary.seller_currency = sellerCurrency;
    }
    const sellerAmount = logging.seller_currency_price as string | number | undefined;
...
function collectDiscount(listing: RawListing): DiscountSummary | undefined {
  const summary: DiscountSummary = {};

  const price = listing.discounted_price as string | number | undefined;
  if (price !== undefined) {
    summary.price = price;
  }
  ...
  const priceUnformatted = listing.discounted_price_unformatted as string | number | undefined;
...
function collectDelivery(listing: RawListing): DeliverySummary | undefined {
  ...
  if (freeShippingData && typeof freeShippingData === "object") {
    summary.free_shipping_data = freeShippingData as Record<string, unknown>;
  }

  const transitManual = listing.transit_details_manual;
  if (Array.isArray(transitManual) && transitManual.length > 0) {
    summary.transit_details_manual = transitManual as Record<string, unknown>[];
  }

  const transitCalculated = listing.transit_details_calculated;
  if (transitCalculated && typeof transitCalculated === "object") {
    summary.transit_details_calculated = transitCalculated as Record<string, unknown>;
...
function collectFlags(listing: RawListing): FlagsSummary | undefined {
  const summary: FlagsSummary = {};

  const pairs: Array<[keyof FlagsSummary, unknown]> = [
...
function collectEngagement(listing: RawListing): EngagementSummary | undefined {
...
function collectVariation(listing: RawListing): VariationSummary | undefined {
  ...
  if (variationsData !== undefined) {
    summary.variations_data = variationsData as Record<string, unknown>;
  }

  const promotions = listing.promotions;
  if (promotions !== undefined) {
    summary.promotions = promotions as Record<string, unknown> | Record<string, unknown>[];
  }

  const promotionData = listing.promotion_data;
  if (promotionData && typeof promotionData === "object") {
    summary.promotion_data = promotionData as Record<string, unknown>;
  }

  const signal = listing.signal_pecking_order;
  if (Array.isArray(signal) && signal.length > 0) {
    summary.signal_pecking_order = signal as Record<string, unknown>[] | string[];
  }

  const styleAttributes = listing.style_attributes;
  if (Array.isArray(styleAttributes) && styleAttributes.length > 0) {
    summary.style_attributes = styleAttributes as Record<string, unknown>[];
  }
  ...
  if (countryPricing && typeof countryPricing === "object") {
    summary.country_specific_pricing = countryPricing as Record<string, unknown>;
  }

  const lowestPrice = listing.lowest_purchasable_price;
  if (lowestPrice && typeof lowestPrice === "object") {
    summary.lowest_purchasable_price = lowestPrice as Record<string, unknown>;
  }

  return Object.keys(summary).length > 0 ? summary : undefined;
}

function collectAttributes(listing: RawListing): Record<string, unknown> | undefined {
...
function toParsedListing(raw: RawListing): ParsedSectionListing | undefined {
  const listingId = raw.listing_id ?? raw.id;
  if (listingId === undefined) {
    return undefined;
  }

  const listing: ParsedSectionListing = {
    listing_id: listingId as string | number,
    raw,
  };
  ...
  if (price !== undefined && price !== null) {
    listing.price = price as string | number;
  }
...
function normalizeResponse(data: unknown): RawListing[] {
  if (Array.isArray(data)) {
    return data as RawListing[];
  }

  if (data && typeof data === "object") {
    const container = data as RawResponseObject;
...
export function parseSectionListings(data: unknown): ParsedSectionListing[] {
  const listings = normalizeResponse(data);
  return listings
    .map((item) => toParsedListing(item))
    .filter((item): item is ParsedSectionListing => Boolean(item));
}
